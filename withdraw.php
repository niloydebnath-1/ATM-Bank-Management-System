...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($withdrawal_amount <= 0 || $withdrawal_amount > $customer['balance']) {
        $error = "Invalid withdrawal amount.";
    } else {
        $new_balance = $customer['balance'] - $withdrawal_amount;
        ...
        if (mysqli_query($conn, $sql_update)) {
            $sql_transaction = "INSERT INTO transactions (account_number, transaction_type, amount, balance_after) 
                                VALUES ({$customer['id']}, 'Withdrawal', $withdrawal_amount, $new_balance)";
            if (mysqli_query($conn, $sql_transaction)) {
                // Redirect back to dashboard after successful withdrawal
